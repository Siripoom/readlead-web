...
export default function DiscoverPage() {
  ...
  const results = useMemo(() => {
    ...
    if (genre !== "all")
      list = list.filter((w) => w.genres.includes(genre as Genre));
    ...
    <div className="mx-auto max-w-7xl px-4 py-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold">
          ค้นพบ{" "}
          <span className="text-muted-foreground font-serif text-lg">发现</span>
        </h1>
        <p className="mt-1 text-sm text-muted-foreground">
          ค้นหาและกรองนิยาย มังงะ และ Audiobook
        </p>
      </div>
      <div className="mb-6 flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-48">
          ...
          <Input
            placeholder="ค้นหาชื่อ ผู้เขียน แท็ก..."
            className="pl-9"
            value={query}
      ...
      <p className="mb-4 text-sm text-muted-foreground">
        พบ <Badge variant="secondary">{results.length}</Badge> รายการ
      </p>
